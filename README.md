...
**Portability TODO:** the shell wrappers hardcode `PROJ=/home/aliu/projects/fail-traj-learn` (WSL side) and
`SCRIPTS=/mnt/c/Users/LocalPC/dev/fail-traj-learn/scripts`. Make both overridable via environment variables before
...
Privileged (oracle only, never fed to policies or critics): `priv.obj_pos`, `priv.obj_quat`
(12 object slots x 3/4, slot names in the sidecar), `priv.obj_valid`, `priv.target_mask`,
`priv.obj_gripper_contact`, `priv.obj_grasped` (both finger pads), `priv.obj_support_contact`
(touching floor/table/fixtures; in LIBERO the tabletop geom is literally named `floor`),
`priv.obj_obj_contact`, `priv.arm_contacts` and `priv.gripper_static_contacts` (collision signals),
`priv.n_contacts`, `priv.eef_pos`, `priv.eef_quat`, `priv.gripper_qpos/qvel`, `priv.joint_pos/vel`,
`priv.sim_time`. Sidecar `episode_XXXXXX.json` carries source policy, task, init protocol, shift
parameters, outcome; `episode_XXXXXX.npz` carries `sim_states` at every chunk boundary.
